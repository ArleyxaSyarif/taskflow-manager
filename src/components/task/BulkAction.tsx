...
export default function BulkAction() {
    ...
    const handleComplete = () => {
        if (window.confirm(`Apakah Anda yakin ingin menyelesaikan ${selectedIds.length} tugas ini?`)) {
            ...
            <div className="flex items-center gap-4 bg-white text-slate-800 px-4 py-3 rounded-full shadow-xl shadow-slate-200/80 border border-slate-200 backdrop-blur-sm bg-opacity-95">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-xs font-semibold px-4 py-1.5 rounded-full transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed shadow-sm hover:shadow-green-600/20"
                    >
                        {bulkComplete.isPending ? (
                            ...
                            <>
                                <svg className="h-3.5 w-3.5 text-green-100" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
